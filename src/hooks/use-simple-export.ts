 ...
 * 临时移除高度限制，让图表内容完全展开
 */
function temporarilyRemoveHeightConstraints(element: HTMLElement): Array<{element: HTMLElement, property: string, value: string}> {
  ...
  allElements.forEach(el => {
    ...
    if (classNameStr.includes('h-[') || classNameStr.includes('h-96') || classNameStr.includes('h-full')) {
 ...
 * 恢复原始样式
 */
function restoreOriginalStyles(element: HTMLElement, originalStyles: Array<{element: HTMLElement, property: string, value: string}>): void {
