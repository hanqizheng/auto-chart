...
export class InputRouter implements IInputRouter {
  ...
  validateInput(
    ...
    try {
      switch (scenario) {
        case "PROMPT_ONLY":
          this.validatePromptOnlyInput(prompt, errors, warnings, conversation);
          break;
   ...
   * 验证仅Prompt场景
   */
  private validatePromptOnlyInput(
    prompt: string,
    errors: string[],
    warnings: string[],
    conversation?: ConversationContextPayload
    ...
    const hasContextChartData =
      !!conversation?.lastChart &&
      Array.isArray(conversation.lastChart.data) &&
      conversation.lastChart.data.length > 0;
    const hasAssistantChartHistory = conversation?.history
      ? conversation.history.some(message => message.kind === "assistant_chart")
      : false;
    const canReuseConversationContext = hasContextChartData || hasAssistantChartHistory;

    if (dataIndicators.length === 0) {
      if (canReuseConversationContext) {
        warnings.push("未检测到新的数据信息，将基于既有图表上下文进行调整");
      } else {
        errors.push(
          "未在描述中检测到具体的数据信息。请提供具体的数值、类别或时间数据，或上传数据文件"
        );
        return;
      }
    }
    ...
    console.log("✅ 仅Prompt场景验证通过，检测到数据指示词:", dataIndicators, {
      reusedContext: canReuseConversationContext,
    });
  }
   ...
   * 验证Prompt+文件场景
   */
  private validatePromptWithFileInput(
    prompt: string,
    files: File[],
    errors: string[],
    warnings: string[]
