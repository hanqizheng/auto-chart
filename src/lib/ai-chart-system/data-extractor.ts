...
重要：只有在用户明确提供了具体数值、列表或表格时才提取数据。`;
  ...
  private async extractFromExcel(file: File): Promise<ExtractedData> {
    return new Promise((resolve, reject) => {
      ...
      reader.onerror = () => reject(new AIChartError(
        'data_extraction',
        'UNKNOWN_ERROR',
        '文件读取失败'
      ));
  ...
  private inferFieldType(values: DataValue[]): FieldType {
    ...
    const dateCount = values.filter(v => v !== null && typeof v === 'string' && !isNaN(Date.parse(v))).length;
  ...
  private calculateQualityScore(data: DataRow[], fields: DataField[]): number {
    ...
    const emptyCells = data.reduce((count, row) => 
      count + fields.reduce((fieldCount, field) => 
        fieldCount + (row[field.name] == null || row[field.name] === '' ? 1 : 0), 0
      ), 0
    );
    ...
    // 数据一致性
    const consistencyScore = fields.reduce((avg, field) => {
  ...
  private inferTimeLabels(prompt: string, length: number): string[] {
    // 检查是否提到了具体的时间标签
    if (/星期|周/.test(prompt)) {
      return ['星期一', '星期二', '星期三', '星期四', '星期五', '星期六', '星期日'].slice(0, length);
    }
    
    if (/月份|月/.test(prompt)) {
      return ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'].slice(0, length);
    }
